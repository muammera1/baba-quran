---
description: Critical operational rules and guidelines for Baba Quran WhatsApp Bot
globs: **/*
---

# Antigravity Rules: Baba Quran Bot

## ⚠️ Critical Operational Rules (DO NOT DEVIATE)

### 1. WhatsApp Dispatch via WA-JS API Only (No DOM Emulation)
- **STRICT RULE**: **NEVER** simulate raw DOM clicks, file dialogs, or keyboard `SendKeys`/`Enter` to send media on WhatsApp Web.
- **MANDATORY PATTERN**: **ALWAYS** use the direct WA-JS API bridge (`src/whatsapp/wppconnect-wa.js`) injected into the persistent Playwright context:
  ...
  ```
  Internally, this invokes `WPP.chat.sendFileMessage(jid, dataUrl, { type: 'image', caption, filename })`.
...
- **STRICT RULE**: Before sending any message or querying reactions via WA-JS:
  ...
  2. Always call `await WPP.chat.find(jid)` and `await WPP.chat.openChatBottom(jid)` to ensure the chat model is loaded into `ChatStore` and avoid `"Chat not found in ChatStore"` errors.
